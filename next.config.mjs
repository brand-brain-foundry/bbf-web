...
const nextConfig = {
  ...
  // B-BBF-WEB-FIX-BUILDID-01: buildId estable por commit — sin esto, Next genera un ID
  // aleatorio en cada build y los Server Actions del bundle viejo mueren al redeploy.
  // GIT_COMMIT_HASH lo inyecta DO App Spec en build-time (scope BUILD_TIME); si no existe
  // (ej. pnpm dev/build local), cae al default de Next (null) sin romper nada.
  generateBuildId: async () => process.env.GIT_COMMIT_HASH ?? null,
